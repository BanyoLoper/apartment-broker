  ...
  return (
    <section style={{ marginTop: 32 }}>
      <div style={{
        ...
        boxShadow: 'var(--ezra-shadow-inset)',
      }}>
        ...
        <div style={{ position: 'absolute', left: 24, bottom: 20, display: 'flex', gap: 8 }}>
          <button onClick={onOpen3D} style={journeyChip('var(--ezra-accent-500)', '#FFFFFF')}>
            <img src="../../assets/icon-3d-room.svg" style={{ width: 16, height: 16, filter: 'brightness(0) invert(1)' }}/>
            Recorrer en 3D
          </button>
          <button onClick={onOpen360} style={journeyChip('rgba(255,255,255,0.92)', 'var(--ezra-ink-900)')}>
            <img src="../../assets/icon-360.svg" style={{ width: 16, height: 16 }}/>
            Vista 360°
          </button>
        </div>
        <div style={{ position: 'absolute', right: 24, bottom: 20, font: 'var(--ezra-text-eyebrow)', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(250,247,242,0.85)' }}>
          01 / Sala
        </div>
      </div>
...
function Frame({ img, label }) {
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', left: 16, bottom: 14, font: 'var(--ezra-text-eyebrow)', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(250,247,242,0.85)' }}>
        {label}
      </div>
    </div>
  );
...
function journeyChip(bg, fg) {
  return {
    ...
    backdropFilter: 'blur(20px) saturate(1.2)', boxShadow: 'var(--ezra-shadow-2)',
  };
}
