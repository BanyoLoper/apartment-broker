  ...
  return (
    <main style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 32px 64px' }}>
      <button onClick={onBack} style={{
        ...
        font: 'var(--ezra-text-body-sm)', color: 'var(--ezra-stone-500)',
      ...
      <header style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 32 }}>
        <div>
          <span className="ezra-eyebrow-wide">{listing.colonia} · piso {listing.floor}</span>
          <h1 className="ezra-display-2" style={{ marginTop: 8, marginBottom: 4 }}>{listing.name}</h1>
          <div style={{ font: 'var(--ezra-text-body)', color: 'var(--ezra-stone-500)', fontFamily: 'var(--ezra-font-mono)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 48, marginTop: 64 }}>
        <div>
          <span className="ezra-eyebrow-wide">Sobre el departamento</span>
          <h2 className="ezra-h2" style={{ marginTop: 8 }}>Una planta luminosa frente a la calle</h2>
          <p style={{ font: 'var(--ezra-text-body)', fontSize: 17, color: 'var(--ezra-stone-500)', marginTop: 16, maxWidth: 560 }}>
          ...
          <div style={{ marginTop: 48 }}>
            <span className="ezra-eyebrow-wide">Comodidades del depa</span>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginTop: 12 }}>
              {['Lavadora · secadora', 'Wi-Fi 1 Gbps', 'Calentador instantáneo', 'Cocina equipada', 'Closet vestidor', 'Persianas blackout', 'Acceso roof', 'Cámara en lobby', 'Bici-estacionamiento'].map(f => (
                <div key={f} style={{
                  font: 'var(--ezra-text-body-sm)', color: 'var(--ezra-ink-900)',
                  display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0',
                }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--ezra-jade-600)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 L9 17 L4 12"/></svg>
                  {f}
                </div>
              ))}
            </div>
...
const iconBtn = {
  ...
  border: '1px solid var(--ezra-border-strong)', color: 'var(--ezra-ink-900)',
  fontFamily: 'inherit', fontSize: 13, fontWeight: 600, cursor: 'pointer',
};

function Feat({ label, v }) {
  return (
    <div style={{ borderTop: '1px solid var(--ezra-border)', paddingTop: 12 }}>
      <div style={{ font: 'var(--ezra-text-eyebrow)', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ezra-fg-muted)' }}>{label}</div>
      <div style={{ fontFamily: 'var(--ezra-font-display)', fontSize: 22, fontWeight: 500, color: 'var(--ezra-ink-900)', marginTop: 4, letterSpacing: '-0.01em' }}>{v}</div>
    </div>
  );
}
