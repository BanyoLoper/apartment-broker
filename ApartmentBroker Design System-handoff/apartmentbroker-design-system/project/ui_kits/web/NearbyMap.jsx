...
function NearbyMap() {
  const categories = [
    { key: 'transit', label: 'Transporte', color: 'var(--ab-info)', count: 8 },
    { key: 'parks', label: 'Parques', color: 'var(--ab-jade-600)', count: 3 },
    { key: 'stores', label: 'Súper · OXXO', color: 'var(--ab-ochre-500)', count: 12 },
    { key: 'cafes', label: 'Cafés', color: 'var(--ab-terracotta-500)', count: 14 },
    { key: 'gyms', label: 'Gimnasios', color: 'var(--ab-rosa-500)', count: 4 },
  ];
  ...
  return (
    <section style={{ marginTop: 64 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <div>
          <span className="ab-eyebrow">Comodidades cercanas</span>
          <h2 className="ab-h2" style={{ marginTop: 8, marginBottom: 0 }}>El barrio, a pie</h2>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {categories.map(c => (
            <button key={c.key} onClick={() => toggle(c.key)} style={{
              ...
              background: active.includes(c.key) ? 'var(--ab-ink-900)' : '#FFFFFF',
              border: `1px solid ${active.includes(c.key) ? 'var(--ab-ink-900)' : 'var(--ab-border-strong)'}`,
              color: active.includes(c.key) ? '#FAF7F2' : 'var(--ab-ink-900)',
              cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600,
            }}>
              <span style={{ width: 8, height: 8, borderRadius: 999, background: c.color }}/>
              {c.label}
              <span style={{ fontFamily: 'var(--ab-font-mono)', fontSize: 11, opacity: 0.7 }}>{c.count}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          backdropFilter: 'blur(10px)', font: 'var(--ab-text-caption)', fontWeight: 600,
          color: 'var(--ab-terracotta-600)',
        }}>
          10 min a pie · 750 m
        </div>
        ...
        <div style={{
          ...
          <div style={{
            padding: '8px 14px', borderRadius: 999, background: 'var(--ab-terracotta-500)',
            color: '#FFFFFF', fontFamily: 'var(--ab-font-mono)', fontSize: 13, fontWeight: 600,
        ...
        {pois.filter(p => active.includes(p.cat)).map((p, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              backdropFilter: 'blur(10px)', fontSize: 11, color: 'var(--ab-ink-900)',
              whiteSpace: 'nowrap', boxShadow: 'var(--ab-shadow-1)',
            }}>
              <span style={{ fontWeight: 600 }}>{p.label}</span>
              <span style={{ color: 'var(--ab-stone-600)', marginLeft: 6, fontFamily: 'var(--ab-font-mono)', fontSize: 10 }}>{p.sub}</span>
            </div>
          </div>
        ))}
      </div>
